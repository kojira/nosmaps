/* Material-style glyphs and the entry's own icon.
   UI layer: produces markup and touches the DOM. */

/** The record fields an icon is derived from. Both are optional because a record
    that never had an icon is the normal case, not a defect. */
export interface IconSource {
  readonly name?: string;
  readonly icon?: {readonly url?: string} | null;
}

const paths: {readonly [name: string]: string | undefined} = {
...
export function svg(name: string, className = 'material-icon'): string {
...
/* with an image (the probe in docs/icons-probe.md, stored with the markup */
/* that declared it in tools/build-data.mjs). A record with `icon: null`   */
/* gets the initial-letter placeholder — no favicon is guessed from a      */
/* homepage, no URL is derived by convention. Absent stays absent.         */
/* ---------------------------------------------------------------------- */
...
function initialOf(tool: IconSource | null | undefined): string {
  const name = typeof tool?.name === 'string' ? tool.name.trim() : '';
  const first = [...name][0];
  return first ? first.toLocaleUpperCase() : '';
}

function placeholderMarkup(initial: string): string {
...
function placeholderElement(initial: string): HTMLSpanElement {
...
export function entity(tool: IconSource | null | undefined): string {
...
export const icons = Object.freeze({svg, entity, names: Object.freeze(Object.keys(paths))});
